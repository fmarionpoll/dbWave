...
public:
	...
	CTemplateWnd* GetTemplateWnd(int i) const { return templatewnd_ptr_array.GetAt(i); }
	CTemplateWnd* GetTemplateWndForClass(int iclass);
	int GetTemplateLength() const { return m_tpllen; }
	int GetNtemplates() const { return GetItemCount(); }
	int GetTemplateclassID(int item) const { return templatewnd_ptr_array.GetAt(item)->m_classID; }
