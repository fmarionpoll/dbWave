...
// Important note: supportLevel field should be incremented by 1 each time a set of parameters are added and
// size of used parameters got altered.

#define DEVICE_CFG_EX_BYTES_USED 71
...
typedef struct ___device_cfg_ex_tag
{
	...
	ULONG		supportLevel;		
	ULONG		uiFlags;
	USHORT		startTrigThresholdTriggerSource;
	DOUBLE		startTrigLevel;
	INT			startTrigPostTriggerScanCount;
	USHORT		referenceTriggerTriggerType;
	USHORT		referenceTrigThresholdTriggerSource;
	DOUBLE		referenceTrigLevel;
	INT			referenceTrigPostTriggerScanCount;
	DOUBLE		excitationVoltageValue;
	ULONG		excitationVoltageSource;
	BOOLEAN		multiRecordAcquisitionEnabled;
	INT			numRecordsToAcquire;
	INT			triggerDelayInSamples;
	INT			retriggerDelayInSamples;
...
// Extended subsystem structure
typedef struct EXTENDED_SStag
{ 
   OLSS SubsystemType;
   INT NumElements;
   VOID        *pSelf;
...
typedef EXTENDED_SS *PEXTENDED_SS;
...
typedef struct tag_SENSOR_CONFIG_ENTRY
{
	IO_TYPE ioType; 
	UINT rtdType;
   ...
   double CurrentVal; // Current Source Value 4m,8m, etc.
	...
	double C;
	double R0;
}SENSOR_CONFIG_ENTRY, *PSENSOR_CONFIG_ENTRY;
...
typedef struct
{
   ...
   BOOL		bWriteInfoToEPROM;
	int numEntries;
   SENSOR_CONFIG_ENTRY sensorConfig[1];
...
#define DT9820EEROM_CLK_REG 3

#define IsExtendedSubsytem(OlSs) ((OlSs) > OLSS_CT)
#define IsValidSubsystem(OlSs) ((OlSs) >= OLSS_AD && (OlSs) <= OLSS_TACH)

#define IsCounterSubsytem(OlSs) (((OlSs) == OLSS_CT) || ((OlSs) == OLSS_TACH))


static double dfAdSupport9820_50Hz[] = 
{
   800.0,         // OLSSCE_MAXTHROUGHPUT   
   6.25,          // OLSSCE_MINTHROUGHPUT
   0.0,           // OLSSCE_MAXRETRIGGER
   0.0,           // OLSSCE_MINRETRIGGER
   1.0,           // OLSSCE_MAXCLOCKDIVIDER
   1.0,           // OLSSCE_MINCLOCKDIVIDER
   4096000       // OLSSCE_BASECLOCK
};

//
// Define the DEVICE_CFG structure for Win64 (thunking).
//
#if defined(_WIN64)

typedef struct ___device_cfg_tag32
{
   // The following 4 fields must be first.  This structure is part of a union and 
   // these fields are accessed thru another representation of that union when this 
   // structure is being used in IOCTL to kernel mode drivers.
...
// LARGE_INTEGER  liFilter[MAX_LEGACY_PER_CHANNEL_FILTERS];  
   ...
   // Misc stuff
   //
   ULONG32           hMfWndNotify;          // Window to notify for Measure Frequency
   DBL            dfCount_Duration;      // Length of time to measure for
   UINT           uiResolution;
