...
//    GridCellNumeric2 class
//  - Modified the Edit function so it creates a CInPlaceNumEdit
//    control instead of a InPlaceEdit control
//
//////////////////////////////////////////////////////////////////////
...
class CInPlaceNumEdit : public InPlaceEdit
{
public:
			...
			LastValid(_T("0")),Updating(0),Sel(0),cDecimal(0),cNegative(0),iNegative(-1),Flags(dwFlags)
{
    ...
    count = GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SNATIVEDIGITS, NULL, 0);
    count = GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SNATIVEDIGITS, sDigits.GetBuffer(count), count);
    sDigits.ReleaseBuffer();
    if (count == 0)
        sDigits = _T("0123456789");

    // Get the locale's decimal point
    if (Flags & GridCellNumeric2::Currency)
    {
        count = GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SMONDECIMALSEP, NULL, 0);
        count = GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SMONDECIMALSEP, buffer.GetBuffer(count),  count);
        buffer.ReleaseBuffer();
        cDecimal = count ? buffer[0] : _T('.');
        sDigits += cDecimal;
    }
    else if (Flags & GridCellNumeric2::Real)
    {
        count = GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SDECIMAL, NULL, 0);
        count = GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SDECIMAL, buffer.GetBuffer(count),  count);
        buffer.ReleaseBuffer();
        cDecimal = count ? buffer[0] : _T('.');
        sDigits += cDecimal;
    }

    if (Flags & GridCellNumeric2::Negative)
    {
        // Get the locale's negative sign
        count = GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SNEGATIVESIGN,NULL,0);
        count = GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SNEGATIVESIGN,buffer.GetBuffer(count),count);
        buffer.ReleaseBuffer();
        ...
        if (Flags & GridCellNumeric2::Currency)
            count = GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_INEGCURR | LOCALE_RETURN_NUMBER,(LPTSTR)&iNegative,sizeof(int) / sizeof(TCHAR));
        else
            count = GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_INEGNUMBER | LOCALE_RETURN_NUMBER,(LPTSTR)&iNegative,sizeof(int) / sizeof(TCHAR));
        if (!count)
            iNegative = 1;
    ...
    int LeadingZero;
    count = GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_ILZERO | LOCALE_RETURN_NUMBER,(LPTSTR)&LeadingZero,sizeof(int) / sizeof(TCHAR));
    if (!count)
        LeadingZero = 1;
    ...
    // InPlaceEdit base class a protected member. It is currently  //
...
BEGIN_MESSAGE_MAP(CInPlaceNumEdit, InPlaceEdit)
...
void CInPlaceNumEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    InPlaceEdit::OnChar(nChar, nRepCnt, nFlags);
}
...
void CInPlaceNumEdit::OnUpdate()
{
    if (!Updating)
    {
        ...
        if (Temp == str)
        {
            ...
            if (Ok && Flags & (GridCellNumeric2::Real | GridCellNumeric2::Currency))
            {
        ...
        else
        {
            ...
            // audible warning
            if (!(Flags & GridCellNumeric2::Silent))
                MessageBeep(MB_OK);
...
void CInPlaceNumEdit::OnKillFocus(CWnd *pNewWnd)
{
    ...
    InPlaceEdit::OnKillFocus(pNewWnd);
}

/////////////////////////////////////////////////////////////////////////////
// class GridCellNumeric2
...
/////////////////////////////////////////////////////////////////////////////
//
//  GridCellNumeric2 constructor  (public member function)
//    Initializes member variables
//
//  Parameters :
//    None
...
GridCellNumeric2::GridCellNumeric2() :
m_pNumberFmt(0),
m_pCurrencyFmt(0),
m_dwFlags(0),
m_nNumber(0.0)
...
BOOL GridCellNumeric2::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    m_bEditing = TRUE;
...
/////////////////////////////////////////////////////////////////////////////
//
//  GridCellNumeric2::SetText  (public virtual member function)
...
void GridCellNumeric2::SetText(LPCTSTR szText)
{
    CString str = szText;
    if (Format(str))
...
/////////////////////////////////////////////////////////////////////////////
//
//  GridCellNumeric2::SetFlags  (public member function)
//    Sets the bit flags that control the type of number and edit control
//    Valid bit flags are:
//             GridCellNumeric2::Integer
//             GridCellNumeric2::Real
//             GridCellNumeric2::Currency
//             GridCellNumeric2::Negative
//             GridCellNumeric2::Silent
//
//  Parameters :
...
DWORD GridCellNumeric2::SetFlags(DWORD dwFlags)
{
...
/////////////////////////////////////////////////////////////////////////////
//
//  GridCellNumeric2::SetNumber  (public member function)
...
BOOL GridCellNumeric2::SetNumber(double Number)
{
    CString str;
...
/////////////////////////////////////////////////////////////////////////////
//
//  GridCellNumeric2::Format  (protected member function)
...
BOOL GridCellNumeric2::Format(CString &TheString)
{
    int count = 0;
...
//    ASSERT (*EndPtr == NULL);
    ...
    TRACE (_T("GridCellNumeric2::Format() - Failed to correctly format string."));
//    ASSERT(0);
    return FALSE;
}
