...
{
	const auto n_tags = m_pChartDataWnd->vt_tags.get_tag_list_size();
	auto tag_first = -1;
	auto tag_last = -1;
	...
	for (auto i = 0; i < n_tags; i++)
	{
		...
		{
			tag_last = i;
			MeasureWithinInterval(channel, line, m_pChartDataWnd->vt_tags.get_tag_value_long(tag_first),
			                      m_pChartDataWnd->vt_tags.get_tag_value_long(tag_last));
			line++;
			tag_first = -1;
			tag_last = -1;
		}
	...
	{
		const auto l1 = m_pChartDataWnd->vt_tags.get_tag_value_long(tag_first);
...
{
	const auto number_of_tags = m_pChartDataWnd->hz_tags.get_tag_list_size();
	auto tag_first = -1;
	auto tag_last = -1;
	...
	for (int i = 0; i < number_of_tags; i++)
	{
		...
		if (tag_last < 0)
		{
			...
			MeasureBetweenHZ(ichan, line,
			                 m_pChartDataWnd->hz_tags.get_value_int(tag_first),
			                 m_pChartDataWnd->hz_tags.get_value_int(tag_last));
			line++;
			tag_first = -1;
			tag_last = -1;
		}
	...
	if (tag_first > 0 && tag_last < 0)
	{
		const auto v1 = m_pChartDataWnd->hz_tags.get_value_int(tag_first);
