...
long CSpikeDoc::BuildPSTH(OPTIONS_VIEWSPIKES* vdS, long* plSum0, int iclass)
{
	...
	if (istimulusindex > m_stimulus_intervals.GetSize() - 1)
		istimulusindex = m_stimulus_intervals.GetSize() - 1;
	if (istimulusindex < 0)
		istimulusindex = 0;
	...
	for (auto istim = istim0; istim < istim1; istim += increment, n++)
	{
		...
		auto i0 = 0;
		while ((i0 < nspikes) && (pspklist->GetSpike(i0)->get_time() < iitime_start))
			i0++;
...
long CSpikeDoc::BuildAUTOCORR(OPTIONS_VIEWSPIKES* vdS, long* plSum0, int iclass)
{
	...
	for (auto istim = istim0; istim < istim1; istim += increment, n++)
	{
		...
		auto i0 = 0;
		while ((i0 < nspikes) && (pspklist->GetSpike(i0)->get_time() < iistart))
			i0++;

		// build histogram external loop search 'pivot spikes'
		for (auto i = i0; i < nspikes; i++)
		{
			const auto iitime0 = pspklist->GetSpike(i)->get_time(); // get spike time
			...
			while (i1 > 0 && pspklist->GetSpike(i1)->get_time() > ifirst)
				i1--;
...
long CSpikeDoc::BuildPSTHAUTOCORR(OPTIONS_VIEWSPIKES* vdS, long* plSum0, int iclass)
{
	...
	for (auto istim = istim0; istim < istim1; istim += increment, n++)
	{
		...
		while (pspklist->GetSpike(i0)->get_time() < iistart)
		{
			i0++; // loop until found
			if (i0 >= nspikes)
				return 0L;
...
SpikeList* CSpikeDoc::GetSpkList_At(int spike_list_index)
{
