...
boolean ViewSpikeDetection::update_data_file(BOOL bUpdateInterface)
{
	...
	if (m_chart_data_filtered.GetChanlistSize() < 1)
	{
		...
		m_chart_data_filtered.m_HZtags.AddTag(m_p_detect_parameters->detect_threshold_bin, 0);
	}
...
LRESULT ViewSpikeDetection::OnMyMessage(WPARAM wParam, LPARAM lParam)
{
	...
	case HINT_MOVEHZTAG:
		m_p_detect_parameters->detect_threshold_bin = m_chart_data_filtered.m_HZtags.GetValue(threshold);
...
void ViewSpikeDetection::update_spike_shape_window_scale(const BOOL b_set_from_controls)
{
	...
	{
		const float x = m_chart_spike_shape.get_extent_ms() / static_cast<float>(m_chart_spike_shape.GetNxScaleCells());
		m_chart_spike_shape.SetxScaleUnitValue(x);

		const float y = m_chart_spike_shape.get_extent_m_v() / static_cast<float>(m_chart_spike_shape.GetNyScaleCells());
...
void ViewSpikeDetection::update_combo_box()
{
	...
	m_chart_data_filtered.m_HZtags.SetTagVal(0, m_p_detect_parameters->detect_threshold_bin);
	m_p_detect_parameters->detect_threshold_mv = m_thresholdval;
}
