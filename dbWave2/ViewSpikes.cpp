...
void ViewSpikes::setAddspikesMode(int mousecursorType)
{
...
{
	const int method = m_pSpkList->GetDetectParms()->detect_transform;
	const int doc_channel = m_pSpkList->GetDetectParms()->extract_channel;
	const int prethreshold = m_pSpkList->GetDetectParms()->detect_pre_threshold;
...
void ViewSpikes::selectSpike(int spikeno)
{
	...
	if (spikeno >= 0 && spikeno < m_pSpkList->GetTotalSpikes())
	{
		...
		const auto spk_first = p_spike_element->get_time() - m_pSpkList->GetDetectParms()->detect_pre_threshold;
...
void ViewSpikes::updateDataFile(BOOL bUpdateInterface)
{
	...
	int isourceview = detect->extract_channel;
...
void ViewSpikes::adjustYZoomToMaxMin(BOOL bForceSearchMaxMin)
{
...
void ViewSpikes::selectSpkList(int icursel)
{
...
void ViewSpikes::PrintFileBottomPage(CDC* p_dc, CPrintInfo* pInfo)
{
...
long ViewSpikes::PrintGetFileSeriesIndexFromPage(int page, int* filenumber)
{
...
CString ViewSpikes::PrintBars(CDC* p_dc, CRect* rect)
{
	CString str_comment;
	const CString rc(_T("\n"));
	CString tab(_T("     "));
	...
	if (options_viewdata->bTimeScaleBar)
	{
		...
		const auto xbar = static_cast<float>(iibar) / m_pSpkDoc->GetAcqRate();
		CString cs;
		cs.Format(_T("\nbar = %f s"), xbar);
		cs_comment += cs;
...
void ViewSpikes::OnPrint(CDC* p_dc, CPrintInfo* pInfo)
{
	...
	for (int i = 0; i < m_nbrowsperpage; i++)
	{
		...
		if (m_pDataDoc != nullptr)
		{
			...
			iextent = m_ChartDataWnd.GetChanlistItem(0)->GetYextent();
...
void ViewSpikes::centerDataDisplayOnSpike(int spikeno)
{
	...
	Spike* p_spike_element = m_pSpkList->GetSpike(spikeno);
	const long spk_first = p_spike_element->get_time() - m_pSpkList->GetDetectParms()->detect_pre_threshold;
...
void ViewSpikes::scrollFile(UINT nSBCode, UINT nPos)
{
	...
	case SB_THUMBTRACK: // drag scroll box -- pos = nPos
		b_result = m_ChartDataWnd.GetDataFromDoc(
			static_cast<long>(nPos) * (m_ChartDataWnd.GetDocumentLast()) / static_cast<long>(100));
		break;
