...
public:
	...
	inline void		SetDataBinFormat(long binzero, long binspan) 
														{ dl_databinzero = binzero; dl_databinspan = binspan; }
	inline void		SetDataVoltsFormat(float VperBin, float VoltsSpan) 
														{ dl_datavoltspbin = VperBin; dl_datavoltspan = VoltsSpan; }
	...
	inline void		SetOrdinatesSourceData(int chan, int transform) 
														{ pEnvelopeOrdinates->SetSourceData(chan, transform); }
};
