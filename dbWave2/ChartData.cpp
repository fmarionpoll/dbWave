...
void ChartData::UpdateGainSettings(int ichan)
{
	CChanlistItem* pchan = chanlistitem_ptr_array[ichan];
...
int ChartData::set_channel_list_source_channel(int i_channel, int acq_channel)
{
	// check if channel is allowed
	const auto pwave_format = m_pDataFile->get_waveformat();
	if (pwave_format->scan_count <= acq_channel || acq_channel < 0)
		return -1;
	...
	// change channel
	const auto p_chanlist_item = chanlistitem_ptr_array[i_channel];
	const auto p_ord = p_chanlist_item->pEnvelopeOrdinates;
	p_ord->SetSourceChan(acq_channel);
	const auto mode = p_ord->GetSourceMode();
	// modify comment
	const auto pchan_array = m_pDataFile->get_wavechan_array();
	const auto pchan = pchan_array->get_p_channel(acq_channel);
	p_chanlist_item->dl_comment = pchan->am_csComment;
	if (mode > 0)
		p_chanlist_item->dl_comment = (m_pDataFile->get_transform_name(mode)).Left(6) + _T(": ") + p_chanlist_item->
			dl_comment;
	UpdateGainSettings(i_channel);
	return acq_channel;
}

void ChartData::set_channel_list_y(int ichan, int acqchan, int transform)
{
	// change channel
...
{
	auto ichanfirst = i_chan;
	auto ichanlast = i_chan;
	if (i_chan < 0)
	{
		ichanfirst = 0;
	...
	for (auto i = ichanfirst; i <= ichanlast; i++)
	{
		...
		if (p_value == nullptr)
			voltsextent = yvoltsperbin * float(p_chanlist_item->GetYextent());
...
void ChartData::set_channel_list_volts_zero(const int i_chan, const float* p_value)
{
	...
	for (auto i = ichanfirst; i <= ichanlast; i++)
	{
		...
		const auto yvoltsperbin = p_chanlist_item->GetVoltsperDataBin();
		if (p_value == nullptr)
			voltsextent = yvoltsperbin * float(p_chanlist_item->GetDataBinZero());
...
int ChartData::set_channel_list_transform_mode(int ichan, int i_mode)
{
	...
	p_chanlist_item->dl_comment = pchan->am_csComment;
	if (i_mode > 0)
		p_chanlist_item->dl_comment = (m_pDataFile->get_transform_name(i_mode)).Left(8)
...
{
	int max, min;
	const auto nchans = get_channel_list_size();
	auto icur = nchans - 1;
...
void ChartData::CenterChan(int j) const
{
	auto i1 = j;
	auto i2 = j;
	if (j < 0)
...
void ChartData::MaxgainChan(int j) const
{
	auto i1 = j;
	auto i2 = j;
	if (j < 0)
	...
	for (auto i = i1; i <= i2; i++)
	{
		CChanlistItem* pchan = get_channel_list_item(i);
...
BOOL ChartData::AttachDataFile(AcqDataDoc* p_data_file)
{
	...
	{
		if (get_channel_list_item(i)->GetSourceChan() > docchanmax)
...
void ChartData::update_x_ruler()
{
	...
	x_ruler.UpdateRange(double(m_lxFirst) / double(m_samplingrate), double(m_lxLast) / double(m_samplingrate));
...
void ChartData::update_y_ruler()
{
	...
	const auto binlow = get_channel_list_y_pixels_to_bin(0, 0);
	const auto binhigh = get_channel_list_y_pixels_to_bin(0, m_client_rect_.Height());

	const CChanlistItem* pchan = get_channel_list_item(0);
	const auto yfirst = double(pchan->ConvertDataBinsToVolts(binlow));
	const auto ylast = double(pchan->ConvertDataBinsToVolts(binhigh));
	
	y_ruler.UpdateRange(yfirst, ylast);
...
void ChartData::plot_data_to_dc(CDC* p_dc)
{
	...
	if (!IsDefined() || m_pDataFile == nullptr)
	{
		...
		p_dc->DrawText(cs_empty_, textlen, rect, DT_LEFT);
		return;
	}
	...
	if (b_bottom_comment)
	{
		const auto textlen = cs_bottom_comment.GetLength();
		p_dc->SetTextColor(RGB(0, 0, 255)); // BLUE
		p_dc->DrawText(cs_bottom_comment, textlen, rect, DT_RIGHT | DT_BOTTOM | DT_SINGLELINE);
	}
	...
	const auto yVE = m_display_rect_.Height();
	...
	{
		const auto poldp = p_dc->SelectObject(&black_dotted_pen_);
...
void ChartData::display_hz_tags_for_channel(CDC* p_dc, int ichan, const CChanlistItem* pChan)
{
	...
	{
		if (horizontal_tags.get_channel(i) != ichan)
			continue;
...
void ChartData::Print(CDC* p_dc, CRect* pRect, BOOL bCenterLine)
{
	...
	const auto yVE = -m_display_rect_.Height();
	const int yVO = m_display_rect_.top + m_display_rect_.Height() / 2;
	const auto xVE = m_display_rect_.Width();
	const int xVO = m_display_rect_.left;
	...
	ResizeChannels(m_display_rect_.Width(), m_lxSize);
	if (!bCenterLine)
		GetDataFromDoc();
	else
	...
	for (auto ichan = chanlistitem_ptr_array.GetUpperBound(); ichan >= 0; ichan--) // scan all channels
	{
		...
		const auto pY = chanlist_item->pEnvelopeOrdinates; // load pointer to ordinates
		...
		if (horizontal_tags.get_tag_list_size() > 0) // print HZ cursors if any?
		{
			...
			{
				if (horizontal_tags.get_channel(j) != ichan) // next tag if not associated with
...
BOOL ChartData::CopyAsText(int ioption, int iunit, int nabcissa)
{
	...
	{
		BeginWaitCursor();
		const auto oldcx = ResizeChannels(nabcissa, 0); 
		GetDataFromDoc();
		EmptyClipboard(); 
		...
		for (auto i = 0; i <= ichans; i++) // scan all channels
		{
			...
			if (iunit == 1)
			{
				lpCopy += wsprintf(lpCopy, _T("%f\t"),
				                   static_cast<double>(k) * double(get_channel_list_item(i)->GetVoltsperDataBin()) * 1000.);
			}
			else
...
void ChartData::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (current_cursor_mode_ == 0 && hc_trapped_ < 0) // test if cursor hits a curve
	{
		...
		if (m_hitcurve >= 0)
		{
			...
			const auto pY = chanlist_item->pEnvelopeOrdinates; // load ordinates
	...
	{
		const auto chanlist_item = chanlistitem_ptr_array[horizontal_tags.get_channel(hc_trapped_)];
		m_y_we_ = chanlist_item->GetYextent(); // store extent
		m_y_wo_ = chanlist_item->GetYzero(); // store zero
	}
}
	...
	case TRACK_VTTAG:
		{
			...
			const auto lval = static_cast<long>(point.x) * (file_position_last_right_pixel_ - file_position_first_left_pixel_ + 1) / static_cast<long>(m_display_rect_.
				right) + file_position_first_left_pixel_;
			vertical_tags.set_tag_l_value(hc_trapped_, lval);
			point.x = static_cast<int>((lval - file_position_first_left_pixel_) * long(m_display_rect_.right) / (file_position_last_right_pixel_ - file_position_first_left_pixel_ + 1));
...
int ChartData::does_cursor_hit_curve(CPoint point)
{
	...
	const auto xextent = chanlist_item->GetnElements();
	...
	for (auto chan = 0; chan <= ichans; chan++) // scan all channels
	{
		...
		const auto ival = get_channel_list_y_pixels_to_bin(chan, point.y);
		const auto ijitter = MulDiv(cy_jitter_, get_channel_list_item(chan)->GetYextent(), -m_y_ve_);
...
void ChartData::SetHighlightData(CDWordArray* pDWintervals)
{
	...
	m_highlighted.channel = int(pDWintervals->GetAt(0));
	m_highlighted.color = pDWintervals->GetAt(1);
	m_highlighted.pensize = int(pDWintervals->GetAt(2));
	...
	{
		m_highlighted.l_first.Add(long(pDWintervals->GetAt(i)));
		m_highlighted.l_last.Add(long(pDWintervals->GetAt(i + 1)));
	}
}
...
void ChartData::load_all_channels(int data_transform)
{
	...
	for (auto channel = 0; channel < n_document_channels; channel++)
	{
		...
		get_channel_list_item(channel)->SetColor(channel);
	}
}
