...
BOOL CMeasureVTtagsPage::GetVTtagVal(int index)
{
	...
	if (index < 0 || index >= m_nbtags)
		return FALSE;
	m_index = index;
	const auto lk = m_pChartDataWnd->vertical_tags.get_tag_l_val(m_index);
...
void CMeasureVTtagsPage::OnEnChangeTimesec()
{
	if (mm_timesec.m_bEntryDone)
	{
		...
		if (m_index >= 0 && m_index < m_nbtags)
		{
			m_pChartDataWnd->vertical_tags.set_tag_l_value(m_index, lk);
			m_pChartDataWnd->Invalidate();
		}
	}
}
...
void CMeasureVTtagsPage::OnShiftTags()
{
	...
	for (auto i = 0; i < m_nbtags; i++)
		m_pChartDataWnd->vertical_tags.set_tag_l_value(i, m_pChartDataWnd->vertical_tags.get_tag_l_val(i) + offset);
	// update data
	m_pChartDataWnd->Invalidate();
	GetVTtagVal(m_index);
	UpdateData(FALSE);
...
void CMeasureVTtagsPage::OnAddTags()
{
	...
	{
		m_nbtags = m_pChartDataWnd->vertical_tags.get_tag_list_size();
		time = m_pChartDataWnd->vertical_tags.get_tag_l_val(m_nbtags - 1) / m_samplingrate;
