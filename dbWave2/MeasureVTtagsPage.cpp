...
BOOL CMeasureVTtagsPage::GetVTtagVal(int index)
{
	...
	if (index < 0 || index >= m_nbtags)
		return FALSE;
	m_index = index;
	const auto lk = m_pChartDataWnd->m_VTtags.get_tag_l_val(m_index);
...
void CMeasureVTtagsPage::OnOK()
{
	...
	p_tag_list->copy_tag_list(&m_pChartDataWnd->m_VTtags);
...
BOOL CMeasureVTtagsPage::OnInitDialog()
{
	...
	m_nbtags = m_pChartDataWnd->m_VTtags.get_tag_list_size();
	GetVTtagVal(0);
...
void CMeasureVTtagsPage::OnRemove()
{
	...
	{
		m_pChartDataWnd->m_VTtags.remove_tag(m_index);
		m_nbtags--;
	}
...
void CMeasureVTtagsPage::OnEnChangeTimesec()
{
	if (mm_timesec.m_bEntryDone)
	{
		...
		if (m_index >= 0 && m_index < m_nbtags)
		{
			m_pChartDataWnd->m_VTtags.set_tag_l_value(m_index, lk);
			m_pChartDataWnd->Invalidate();
		}
	}
}
...
void CMeasureVTtagsPage::OnShiftTags()
{
	...
	for (auto i = 0; i < m_nbtags; i++)
		m_pChartDataWnd->m_VTtags.set_tag_l_value(i, m_pChartDataWnd->m_VTtags.get_tag_l_val(i) + offset);
	// update data
	m_pChartDataWnd->Invalidate();
	GetVTtagVal(m_index);
	UpdateData(FALSE);
...
void CMeasureVTtagsPage::OnAddTags()
{
	...
	{
		m_nbtags = m_pChartDataWnd->m_VTtags.get_tag_list_size();
		time = m_pChartDataWnd->m_VTtags.get_tag_l_val(m_nbtags - 1) / m_samplingrate;
		time_end = m_period * static_cast<float>(m_nperiods) + time;
		// delete this one which will be re-created within the loop
		m_pChartDataWnd->m_VTtags.remove_tag(m_nbtags - 1);
		m_nbtags--;
	}
	...
	while (time <= time_end)
	{
		m_pChartDataWnd->m_VTtags.add_l_tag(static_cast<long>(time * m_samplingrate), 0);
		m_pChartDataWnd->m_VTtags.add_l_tag(static_cast<long>((time + m_duration) * m_samplingrate), 0);
		n_intervals++;
...
void CMeasureVTtagsPage::OnDeleteSeries()
{
	...
	m_pChartDataWnd->m_VTtags.copy_tag_list(p_tags_list);
