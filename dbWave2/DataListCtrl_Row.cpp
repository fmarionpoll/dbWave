...
void DataListCtrl_Row::display_data_wnd(DataListCtrlInfos* infos, const int i_image)
{
	...
	if (cs_datafile_name.IsEmpty() || !p_data_doc->open_document(cs_datafile_name))
	{
		...
		p_data_chart_wnd->set_string(comment);
	}
	else
	{
		if (cs_n_spikes.IsEmpty())
			p_data_chart_wnd->get_scope_parameters()->cr_scope_fill = ChartData::get_color(2);
		else
			p_data_chart_wnd->get_scope_parameters()->cr_scope_fill = ChartData::get_color(15);

		p_data_doc->read_data_infos();
		cs_comment = p_data_doc->get_wave_format()->get_comments(_T(" "));
		p_data_chart_wnd->attach_data_file(p_data_doc);
		p_data_chart_wnd->load_all_channels(infos->data_transform);
		p_data_chart_wnd->load_data_within_window(infos->b_set_time_span, infos->t_first, infos->t_last);
		p_data_chart_wnd->adjust_gain(infos->b_set_mv_span, infos->mv_span);
...
void DataListCtrl_Row::display_spike_wnd(DataListCtrlInfos* infos, const int i_image)
{
	...
	if (p_spike_chart_wnd == nullptr)
	{
		p_spike_chart_wnd = new ChartSpikeBar;
	...
	else
	{
		...
		if (infos->b_set_mv_span)
		{
			...
			p_spike_chart_wnd->set_yw_ext_org(y_we, y_wo);
		}
