...
DlgDataViewOrdinates::DlgDataViewOrdinates(CWnd* pParent /*=NULL*/)
	: CDialog(IDD, pParent)	  
{
}

void DlgDataViewOrdinates::DoDataExchange(CDataExchange* pDX)
{
...
BOOL DlgDataViewOrdinates::OnInitDialog()
{
	...
	int i;
	m_nChanmax = m_pChartDataWnd->get_channel_list_size(); // nb of data channels
	for (i = 0; i < m_nChanmax; i++) // browse through all chans
	{
		CChanlistItem* chan = m_pChartDataWnd->get_channel_list_item(i);
		m_settings.Add(chan->get_y_zero()); // save zero
		m_settings.Add(chan->get_y_extent()); // save extent
	}
	...
	{
		CChanlistItem* chan = m_pChartDataWnd->get_channel_list_item(i);
		chan->set_y_zero(m_settings.GetAt(j));
		j++; // restore zero
		chan->set_y_extent(m_settings.GetAt(j));
		j++; // restore extent
		m_chanSelect.AddString(chan->get_comment()); // load comment/chan
	}
...
void DlgDataViewOrdinates::LoadChanlistData(int i)
{
	CChanlistItem* chan = m_pChartDataWnd->get_channel_list_item(i);
	// compute max and min from zero and extent
	m_VoltsperBin = chan->get_volts_per_bin();
...
void DlgDataViewOrdinates::SaveChanlistData(int indexlist)
{
...
void DlgDataViewOrdinates::ChangeUnits(int iUnit, BOOL bNew)
{
	auto newp10 = m_p10;
	if (bNew)
	{
...
void DlgDataViewOrdinates::OnSelchangeVertunits()
{
...
void DlgDataViewOrdinates::OnSelchangeChanselect()
{
	if (m_bChanged)
...
void DlgDataViewOrdinates::OnKillfocusVertcenter()
{
	auto diff = m_xcenter;
	UpdateData(TRUE);
	if (diff == m_xcenter)
	...
	CChanlistItem* chan = m_pChartDataWnd->get_channel_list_item(m_Channel);
	const auto vper_bin = chan->get_volts_per_bin();
	const auto xzero = ((m_xmax + m_xmin) / 2.f) * m_p10;
...
void DlgDataViewOrdinates::OnKillfocusVertMxMi()
{
	UpdateData(TRUE);
...
void DlgDataViewOrdinates::OnCancel()
{
	...
	{
		CChanlistItem* chan = m_pChartDataWnd->get_channel_list_item(i);
		chan->set_y_zero(m_settings[j]);
		j++;
		chan->set_y_extent(m_settings[j]);
		j++;
	}
	CDialog::OnCancel();
}
