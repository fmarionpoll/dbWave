...
void OPTIONS_VIEWSPIKES::Serialize(CArchive & ar)
{
	if (ar.IsStoring())
	{
		...
		wM = 1;	  wE = bacqcomments * wM;		//1
		wM += wM; wE += bacqdate * wM;			//2
		wM += wM; wE += bacqcomments * wM;		//4
		wM += wM; wE += bacqdate * wM;			//8
		wM += wM; wE += bacqchsettings * wM;	//16
		wM += wM; wE += bspkcomments * wM;		//32
		wM += wM; wE += bincrflagonsave * wM;	//64
		wM += wM; wE += bexportzero * wM;		//128
		wM += wM; wE += bexportPivot * wM;		//256
		wM += wM; wE += bexporttoExcel * wM;	//512	empty slot
		wM += wM; wE += bartefacts * wM;		//1024
		wM += wM; wE += bcolumnheader * wM;		//2048
		wM += wM; wE += btotalspikes * wM;		//4096
		wM += wM; wE += babsolutetime * wM;		//8192
		wM += wM; wE += ballfiles * wM;			//16384
		wM += wM; wE += bdotunderline * wM;		//32768
		ar << wE;

		// int parameters
		WORD wIntParms = 11; ar << wIntParms;		// set to nb of following lines
