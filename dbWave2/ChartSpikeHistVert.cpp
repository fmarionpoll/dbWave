...
void ChartSpikeHistVert::PlotDataToDC(CDC* p_dc)
{
	...
	for (auto ihist = 0; ihist < histogram_ptr_array.GetSize(); ihist++)
	{
		...
		if (ihist > 0)
		{
			...
			color = BLACK_COLOR;
			if (PLOT_ONECLASSONLY == m_plotmode && spkcla != m_selected_class)
				continue;
			if (PLOT_CLASSCOLORS == m_plotmode)
				color = spkcla % NB_COLORS;
			else if (m_plotmode == PLOT_ONECLASS && spkcla == m_selected_class)
			{
				color = BLACK_COLOR;
				continue;
			}
                                                ...
                                                int min, int nbins, BOOL bNew)
{
	...
	for (auto ifile = file_first; ifile <= file_last; ifile++)
	{
		if (ballFiles)
		{
			p_doc->set_db_current_record_position(ifile);
	...
	if (ballFiles)
	{
		p_doc->set_db_current_record_position(currentfile);
		p_doc->open_current_spike_file();
	}
}
